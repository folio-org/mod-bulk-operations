...
class BulkEditItemSkipListenerTest {

  @Mock private BulkEditSkipListener bulkEditSkipListener;
  @InjectMocks private BulkEditItemSkipListener skipListener;

  @Test
  void onSkipInProcess_shouldDelegateToBulkEditSkipListenerWhenBulkEditException() {
    var identifier = new ItemIdentifier().withItemId("item-1");
    var exception = new BulkEditException("item error", ErrorType.ERROR);
  ...
  void onSkipInProcess_shouldNotDelegateWhenNotBulkEditException() {
    var identifier = new ItemIdentifier().withItemId("item-2");
