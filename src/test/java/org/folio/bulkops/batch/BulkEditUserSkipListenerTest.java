...
class BulkEditUserSkipListenerTest {

  @Mock private BulkEditSkipListener bulkEditSkipListener;
  @InjectMocks private BulkEditUserSkipListener skipListener;

  @Test
  void onSkipInProcess_shouldDelegateToBulkEditSkipListenerWhenBulkEditException() {
    var identifier = new ItemIdentifier().withItemId("user-1");
    var exception = new BulkEditException("user error", ErrorType.ERROR);
  ...
  void onSkipInProcess_shouldNotDelegateWhenNotBulkEditException() {
    var identifier = new ItemIdentifier().withItemId("user-2");
