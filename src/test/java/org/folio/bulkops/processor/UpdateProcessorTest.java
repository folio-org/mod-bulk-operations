...
class UpdateProcessorTest extends BaseTest {
  ...
  @MockitoSpyBean private ItemClient itemClient;
  ...
  @Autowired private HoldingsUpdateProcessor holdingsUpdateProcessor;
  @Autowired private ItemUpdateProcessor itemUpdateProcessor;
  @Autowired private UserUpdateProcessor userUpdateProcessor;
  @Autowired private FolioInstanceUpdateProcessor folioInstanceUpdateProcessor;
  ...
  @MockitoBean private SearchConsortium searchConsortium;
  ...
  void shouldUpdateHoldingsRecordWithTenant() {
    try (var context = new FolioExecutionContextSetter(folioExecutionContext)) {
      doNothing()
          .when(permissionsValidator)
          .checkIfBulkEditWritePermissionExists(
              eq("tenantId"), eq(EntityType.HOLDINGS_RECORD), anyString());
      when(ruleService.getRules(isA(UUID.class))).thenReturn(new BulkOperationRuleCollection());
      var holdingsRecord =
          new HoldingsRecord()
              .withId(UUID.randomUUID().toString())
  ...
  @CsvSource(
      textBlock =
          ...
          """,
      delimiter = '|')
  void holdings_shouldNotUpdateAssociatedItemsDiscoverySuppress(
      UpdateActionType actionType, boolean notChanged) {
    var holdingsId = UUID.randomUUID().toString();
    var holdingsRecord = HoldingsRecord.builder().id(holdingsId).discoverySuppress(true).build();
    var extendedHoldingsRecord = ExtendedHoldingsRecord.builder().entity(holdingsRecord).build();

    var operationId = UUID.randomUUID();
    var operation =
        BulkOperation.builder().id(operationId).identifierType(IdentifierType.ID).build();
    ...
    when(itemClient.getByQuery(expectedQuery, Integer.MAX_VALUE))
        .thenReturn(
            ItemCollection.builder()
                .items(
                    List.of(
                        Item.builder().discoverySuppress(true).build(),
    ...
    if (notChanged) {
      verify(errorService)
          .saveError(
              any(UUID.class), anyString(), eq(MSG_NO_CHANGE_REQUIRED), eq(ErrorType.WARNING));
    } else {
      verify(errorService, times(0))
          .saveError(any(UUID.class), anyString(), anyString(), eq(ErrorType.ERROR));
    }
  }
  ...
  @CsvSource(
      textBlock =
          ...
          """,
      delimiter = '|')
  void holdings_shouldUpdateAssociatedItemsDiscoverySuppress(
      UpdateActionType actionType, boolean notChanged) {
    var holdingsId = UUID.randomUUID().toString();
    var holdingsRecord =
                    ...
                    .actions(
                        Collections.singletonList(
                            new Action()
                                ...
                                .parameters(
                                    Collections.singletonList(
                                        new Parameter().key(APPLY_TO_ITEMS).value("true"))))));
    when(ruleService.getRules(operationId))
        .thenReturn(
            new BulkOperationRuleCollection()
                .bulkOperationRules(Collections.singletonList(rule))
    ...
    if (notChanged) {
      var errorMessage =
          String.format(
              "No change in value for holdings record required, associated %s item(s) have "
                  + "been updated.",
              SET_TO_TRUE_INCLUDING_ITEMS.equals(actionType) ? "unsuppressed" : "suppressed");
      verify(errorService)
          .saveError(any(UUID.class), anyString(), eq(errorMessage), eq(ErrorType.WARNING));
    } else {
      verify(errorService, times(0))
          .saveError(any(UUID.class), anyString(), anyString(), eq(ErrorType.ERROR));
    }
  ...
  @CsvSource(
      textBlock =
          ...
          """,
      delimiter = '|')
  void instance_testUpdateUnderlyingHoldingsDiscoverySuppress(
      UpdateActionType actionType, boolean applyToHoldings, String sourceId) {
    var instanceId = UUID.randomUUID().toString();
    var instance =
        Instance.builder()
            .id(instanceId)
    ...
    when(holdingsReferenceService.getSourceById("marc_id"))
        .thenReturn(HoldingsRecordsSource.builder().name("MARC").build());
    when(holdingsReferenceService.getSourceById("folio_id"))
        .thenReturn(HoldingsRecordsSource.builder().name("FOLIO").build());

    when(holdingsStorageClient.getByQuery(
            String.format(GET_HOLDINGS_BY_INSTANCE_ID_QUERY, instanceId), Integer.MAX_VALUE))
        .thenReturn(
  ...
  @CsvSource(
      textBlock =
          ...
          """,
      delimiter = '|')
  void instance_testUpdateUnderlyingItemsDiscoverySuppress(
      UpdateActionType actionType, boolean applyToItems, String sourceId) {
    var instanceId = UUID.randomUUID().toString();
    var instance =
        Instance.builder()
            .id(instanceId)
    ...
    when(itemClient.getByQuery(
            String.format(GET_ITEMS_BY_HOLDING_ID_QUERY, holdingsId), Integer.MAX_VALUE))
        .thenReturn(
            ItemCollection.builder()
                .items(
                    List.of(
    ...
    try (var context = new FolioExecutionContextSetter(folioExecutionContext)) {
      ...
      var holdingRecord =
          HoldingsRecord.builder()
              ...
              .discoverySuppress(false)
              .build();
      var consortiumHolding =
          new ConsortiumHolding(holdingRecord.getId(), "memberTenant", instanceId);
      var consortiumHolding2 =
          new ConsortiumHolding(UUID.randomUUID().toString(), "memberTenant2", instanceId);
      ...
      var user =
          User.builder()
              .id(folioExecutionContext.getUserId().toString())
              .username("username")
              .build();
      ...
      when(searchConsortium.getHoldingsById(UUID.fromString(instanceId)))
      ...
      when(itemClient.getByQuery(
              String.format(GET_ITEMS_BY_HOLDING_ID_QUERY, holdingRecord.getId()),
              Integer.MAX_VALUE))
          .thenReturn(itemCollection);
      var instance =
          Instance.builder().id(instanceId).source("FOLIO").discoverySuppress(true).build();
      var operation =
          BulkOperation.builder().id(operationId).identifierType(IdentifierType.ID).build();
      var extendedInstance = ExtendedInstance.builder().entity(instance).build();
      folioInstanceUpdateProcessor.updateAssociatedRecords(extendedInstance, operation, false);
  ...
  @Test
  void testSaveWarningWhenInstanceNotChanged() {
    var instanceId = UUID.randomUUID().toString();
    var instance =
        Instance.builder().id(instanceId).source("FOLIO").discoverySuppress(true).build();
    var extendedInstance = ExtendedInstance.builder().entity(instance).build();
    ...
    when(ruleService.getRules(operationId))
        .thenReturn(
            new BulkOperationRuleCollection().bulkOperationRules(Collections.singletonList(rule)));

    folioInstanceUpdateProcessor.updateAssociatedRecords(extendedInstance, operation, true);

    verify(errorService)
        .saveError(eq(operationId), eq(instanceId), anyString(), eq(ErrorType.WARNING));
  }
}
