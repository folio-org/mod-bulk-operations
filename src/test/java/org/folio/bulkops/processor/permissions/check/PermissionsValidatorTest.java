...
class PermissionsValidatorTest {
  ...
  void setup() {
    ReflectionTestUtils.setField(
        readPermissionsValidator, "requiredPermissionResolver", requiredPermissionResolver);
    ReflectionTestUtils.setField(
        readPermissionsValidator, "folioExecutionContext", folioExecutionContext);
    ReflectionTestUtils.setField(
        readPermissionsValidator, "permissionsProvider", permissionsProvider);
  }
  ...
  void testCheckIfBulkEditWritePermissionExistsForInventory() {
    when(permissionsProvider.getUserPermissions(eq("tenant1"), isA(UUID.class)))
        .thenReturn(
            List.of(
                INVENTORY_ITEMS_ITEM_PUT.getValue(),
                "not_write_permission",
    ...
    assertThrows(
        WritePermissionDoesNotExist.class,
        () ->
            permissionsValidator.checkIfBulkEditWritePermissionExists(
                "tenant2", EntityType.ITEM, "errorMessage"));
  }
  ...
  void testCheckIfBulkEditWritePermissionExistsForUsers() {
    ...
    assertThrows(
        ...
        () ->
            permissionsValidator.checkIfBulkEditWritePermissionExists(
                "tenant2", EntityType.USER, "errorMessage"));
  }
  ...
  void shouldThrowExceptionIfBulkEditReadPermissionNotExistsForCurrentTenant(
    ...
    when(userClient.getUserById(userId.toString()))
        .thenReturn(User.builder().id(userId.toString()).username("username").build());
    ...
    var entityWithNotAllowedTenant =
        new ExtendedHoldingsRecord()
            .withTenantId("tenant1")
            .withEntity(HoldingsRecord.builder().id(UUID.randomUUID().toString()).build());
    assertThrows(
        ReadPermissionException.class,
        () -> permissionsValidator.checkPermissions(operation, entityWithNotAllowedTenant));
  }

  @ParameterizedTest
  @CsvSource(
      value = {
        "HOLDINGS_RECORD,INVENTORY_STORAGE_HOLDINGS_ITEM_GET_PERMISSION"
  ...
  void shouldNotThrowExceptionIfBulkEditReadPermissionExistsForCurrentTenant(
      EntityType entityType,
      PermissionEnum inventoryOrUsersPermission,
      PermissionEnum bulkEditPermission) {
    var userId = UUID.randomUUID();

    when(permissionsProvider.getUserPermissions(eq("tenant1"), isA(UUID.class)))
        .thenReturn(List.of(bulkEditPermission.getValue(), inventoryOrUsersPermission.getValue()));
    when(folioExecutionContext.getUserId()).thenReturn(userId);
    when(folioExecutionContext.getTenantId()).thenReturn("tenant1");
    if (entityType == EntityType.ITEM || entityType == EntityType.HOLDINGS_RECORD) {
      when(consortiaService.getCentralTenantId("tenant1")).thenReturn("central");
  ...
  void shouldCallTenantResolverForHoldingsIfCurrentTenantIsCentral() {
    ...
    verify(tenantResolver)
        .checkAffiliatedPermittedTenantIds(
            any(EntityType.class), any(String.class), any(Set.class), any(String.class));
  }
  ...
  void shouldThrowWhenUserLacksLockPermission() {
    ...
    var exception =
        assertThrows(
            ProfileLockedException.class, () -> permissionsValidator.checkIfLockPermissionExists());
