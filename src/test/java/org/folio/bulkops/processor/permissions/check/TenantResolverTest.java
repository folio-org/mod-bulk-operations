...
class TenantResolverTest {
  ...
  void shouldNotThrowAffiliationExceptionForSupportedEntityTypesIfTenantIsAffiliated(
    ...
    when(readPermissionsValidator.isBulkEditReadPermissionExists(currentTenant, entityType))
        .thenReturn(true);
  ...
  void savesErrorWhenTenantNotAffiliatedOrPermissionMissing(
    ...
    verify(errorService, times(tenantIds.size()))
        .saveError(
            any(),
            eq("item123"),
            eq(expectedErrorMessage),
            eq(org.folio.bulkops.domain.dto.ErrorType.ERROR));
  ...
  private static Stream<Arguments> validTenantScenarios() {
    return Stream.of(
        Arguments.of(
            EntityType.ITEM, "ID", Set.of("tenant1", "tenant2"), Set.of("tenant1", "tenant2")),
