...
class MarcCsvHelperTest extends BaseTest {
  ...
  void shouldEnrichCommittedCsvWithUpdatedMarcRecords() {
    ...
    assertThat(pathCaptor.getValue()).isEqualTo("somedir/enriched/committed.csv");
    assertThat(bulkOperation.getLinkToCommittedRecordsCsvFile()).isEqualTo("somedir/enriched/committed.csv");
  }
  ...
  void shouldEnrichCommittedMarcWithUpdatedInventoryRecords() {
    ...
    verify(remoteFileSystemClient).put(contentCaptor.capture(), pathCaptor.capture());
    assertThat(contentCaptor.getValue()).isInstanceOf(SequenceInputStream.class);
    assertThat(pathCaptor.getValue()).isEqualTo("somedir/enriched/committed.mrc");
    assertThat(bulkOperation.getLinkToCommittedRecordsMarcFile()).isEqualTo("somedir/enriched/committed.mrc");
  }
