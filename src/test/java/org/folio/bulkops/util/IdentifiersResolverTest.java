...
class IdentifiersResolverTest {
  ...
  void testResolve() {
    ...
    assertEquals("holdingsRecordId", IdentifiersResolver.resolve(IdentifierType.HOLDINGS_RECORD_ID));
    assertEquals("username", IdentifiersResolver.resolve(IdentifierType.USER_NAME));
    assertEquals("externalSystemId", IdentifiersResolver.resolve(IdentifierType.EXTERNAL_SYSTEM_ID));
