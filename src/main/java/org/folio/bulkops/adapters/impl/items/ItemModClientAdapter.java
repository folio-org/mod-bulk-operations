...
public class ItemModClientAdapter implements ModClient<Item> {
  ...
  public UnifiedTable convertEntityToUnifiedTable(Item item, UUID bulkOperationId, IdentifierType identifierType) {
    ...
    return new UnifiedTable().header(ItemHeaderBuilder.getHeaders()).addRowsItem(convertToUnifiedTableRow(item, bulkOperationId, identifier));
  }

  @Override
  public UnifiedTable getUnifiedRepresentationByQuery(String query, long offset, long limit) {
    var items = itemClient.getItemByQuery(query, offset, limit).getItems();
    return new UnifiedTable()
      .header(ItemHeaderBuilder.getHeaders())
      .rows(items.isEmpty() ?
  ...
  private Row convertToUnifiedTableRow(Item item, UUID bulkOperationId, String identifier) {
    return new Row()
      ...
      .addRowItem(isEmpty(item.getDiscoverySuppress()) ? EMPTY : item.getDiscoverySuppress().toString())
      ...
      .addRowItem(isNull(item.getStatus()) ? EMPTY : String.join(ARRAY_DELIMITER, item.getStatus().getName().getValue(), dateToString(item.getStatus().getDate())))
      .addRowItem(isNull(item.getMaterialType()) ? EMPTY : item.getMaterialType().getName())
      .addRowItem(isNull(item.getIsBoundWith()) ? EMPTY : item.getIsBoundWith().toString())
      .addRowItem(fetchBoundWithTitles(item))
      .addRowItem(isEmpty(item.getPermanentLoanType()) ? EMPTY : item.getPermanentLoanType().getName())
      .addRowItem(isEmpty(item.getTemporaryLoanType()) ? EMPTY : item.getTemporaryLoanType().getName())
      .addRowItem(isEmpty(item.getPermanentLocation()) ? EMPTY : item.getPermanentLocation().getName())
      .addRowItem(isEmpty(item.getTemporaryLocation()) ? EMPTY : item.getTemporaryLocation().getName())
      .addRowItem(isEmpty(item.getEffectiveLocation()) ? EMPTY : item.getEffectiveLocation().getName())
      .addRowItem(itemReferenceResolver.getServicePointNameById(item.getInTransitDestinationServicePointId(), bulkOperationId, identifier))
      .addRowItem(fetchStatisticalCodes(item, bulkOperationId, identifier))
      .addRowItem(item.getPurchaseOrderLineIdentifier())
      .addRowItem(isEmpty(item.getTags().getTagList()) ? EMPTY : String.join(ARRAY_DELIMITER, item.getTags().getTagList()))
      .addRowItem(lastCheckInToString(item, bulkOperationId, identifier))
      .addRowItem(electronicAccessStringMapper.getElectronicAccessesToString(item.getElectronicAccess(), bulkOperationId, identifier));
  }
  ...
  private String effectiveCallNumberComponentsToString(EffectiveCallNumberComponents components, UUID bulkOperationId, String identifier) {
