...
public class HoldingModClientAdapter implements ModClient<HoldingsRecord> {
  ...
  @Override
  public UnifiedTable convertEntityToUnifiedTable(HoldingsRecord holdingsRecord, UUID bulkOperationId, IdentifierType identifierType) {
    var identifier = IdentifierType.ID.equals(identifierType) ? holdingsRecord.getId() : holdingsRecord.getHrid();
    return new UnifiedTable().header(HoldingsHeaderBuilder.getHeaders()).addRowsItem(convertToUnifiedTableRow(holdingsRecord, bulkOperationId, identifier));
  }

  @Override
  public UnifiedTable getUnifiedRepresentationByQuery(String query, long offset, long limit) {
    var holdings = holdingClient.getHoldingsByQuery(query, offset, limit).getHoldingsRecords();
    return new UnifiedTable()
      .header(HoldingsHeaderBuilder.getHeaders())
      .rows(holdings.isEmpty() ?
  ...
  private Row convertToUnifiedTableRow(HoldingsRecord holdingsRecord, UUID bulkOperationId, String identifier) {
    return new Row()
      .addRowItem(holdingsRecord.getId())
      ...
      .addRowItem(holdingsReferenceResolver.getHoldingsTypeNameById(holdingsRecord.getHoldingsTypeId(), bulkOperationId, identifier))
      .addRowItem(isEmpty(holdingsRecord.getFormerIds()) ? EMPTY : String.join(ARRAY_DELIMITER, holdingsRecord.getFormerIds()))
      .addRowItem(isEmpty(holdingsRecord.getInstanceId()) ? EMPTY : String.join(ARRAY_DELIMITER, holdingsReferenceResolver.getInstanceTitleById(holdingsRecord.getInstanceId()), holdingsRecord.getInstanceId()))
      ...
      .addRowItem(electronicAccessStringMapper.getElectronicAccessesToString(holdingsRecord.getElectronicAccess(), bulkOperationId, identifier))
      .addRowItem(holdingsReferenceResolver.getCallNumberTypeNameById(holdingsRecord.getCallNumberTypeId(), bulkOperationId, identifier))
      ...
      .addRowItem(isEmpty(holdingsRecord.getAdministrativeNotes()) ? EMPTY : String.join(ARRAY_DELIMITER, holdingsRecord.getAdministrativeNotes()))
  ...
  private String holdingsStatementsToString(List<HoldingsStatement> statements) {
    return isEmpty(statements) ? EMPTY : statements.stream()
      .map(statement -> String.join(ARRAY_DELIMITER, statement.getStatement(),
        statement.getNote(), statement.getStaffNote()))
      .collect(Collectors.joining(ITEM_DELIMITER));
  }
