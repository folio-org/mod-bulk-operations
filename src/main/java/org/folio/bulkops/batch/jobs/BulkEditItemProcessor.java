...
public class BulkEditItemProcessor
    implements ItemProcessor<ItemIdentifier, ExtendedItemCollection>, EntityExtractor {
  ...
  @Override
  public ExtendedItemCollection process(@NotNull ItemIdentifier itemIdentifier)
    ...
    try {
      final ExtendedItemCollection extendedItemCollection =
          new ExtendedItemCollection().withExtendedItems(new ArrayList<>()).withTotalRecords(0);
      var centralTenantId =
      ...
      if (isCurrentTenantCentral(centralTenantId)) {
        ...
        if (consortiumItemCollection != null) {
          Integer totalRecords = consortiumItemCollection.getTotalRecords();
          if (totalRecords != null && totalRecords > 0) {
            ...
            affiliatedPermittedTenants.forEach(
                tenantId ->
                    processWithAffiliatedPermittedTenants(
                        idType,
                        identifier,
                        identifierType,
                        limit,
        ...
        if (itemCollection.getItems().size() > limit) {
          log.error(
              "Member/local tenant case: response from {} for tenant {}: {}",
              query,
              currentTenantId,
        ...
        var extendedItems =
            itemCollection.getItems().stream()
                .map(
                    item ->
                        item.withTitle(
  ...
  private void processWithAffiliatedPermittedTenants(
      String idType,
      String identifier,
  ...
  private List<ExtendedItem> enrichItemsWithHoldingsData(
    ...
    return itemCollection.getItems().stream()
        .map(
            item ->
                item.withTitle(
                    holdingsReferenceService.getInstanceTitleByHoldingsRecordId(
        ...
        .map(
            item ->
                item.withHoldingsData(
                    holdingsReferenceService.getHoldingsData(item.getHoldingsRecordId(), tenantId)))
        .map(
            item -> {
  ...
  private void checkReadPermissions(String tenantId, String identifier) {
    if (!permissionsValidator.isBulkEditReadPermissionExists(tenantId, EntityType.ITEM)) {
      ...
      throw new BulkEditException(
          NO_ITEM_VIEW_PERMISSIONS.formatted(
              user.getUsername(), resolveIdentifier(identifierType), identifier, tenantId),
          ErrorType.ERROR);
    }
  }
