...
public class RuleUtils {
  ...
  public static Map<String, String> fetchParameters(BulkOperationRule rule) {
    return rule.getRuleDetails().getActions().stream()
        ...
        .collect(
            Collectors.toMap(
                Parameter::getKey, Parameter::getValue, (existing, replacement) -> existing));
  }

  public static String getTenantFromAction(
      Action action, FolioExecutionContext folioExecutionContext) {
    ...
    return nonNull(actionTenants) && !actionTenants.isEmpty()
        ? actionTenants.getFirst()
