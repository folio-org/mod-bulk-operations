...
public class ItemPermissionChecker {
  ...
  public void checkPermissionsAndAffiliations(String itemIdentifier) throws UploadFromQueryException {
    ...
    if (isCurrentTenantCentral(centralTenantId)) {
      ...
      var batchIdsDto = new org.folio.bulkops.domain.dto.BatchIdsDto()
      ...
      if (!consortiumItemCollection.getItems().isEmpty()) {
        ...
        tenantResolver.checkAffiliatedPermittedTenantIds(EntityType.ITEM, IdentifierType.ID.getValue(), tenantIds, itemIdentifier);
      } else {
  ...
  private void checkReadPermissions(String tenantId, String identifier) throws ReadPermissionException {
    if (!permissionsValidator.isBulkEditReadPermissionExists(tenantId, EntityType.ITEM)) {
      ...
      throw new ReadPermissionException(NO_ITEM_VIEW_PERMISSIONS.formatted(user.getUsername(), IdentifierType.ID, identifier, tenantId), identifier);
    }
  }
