    ...
    return (option, action) -> {
      if (EXPIRATION_DATE == option) {
        if (action.getType() != REPLACE_WITH) {
          throw new RuleValidationException(String.format("Action %s cannot be applied. The only REPLACE_WITH is supported.", action.getType()));
        ...
        if (REPLACE_WITH != action.getType()) {
          ...
          throw new RuleValidationException("Updated value cannot be null or empty");
        } else if (isEmpty(groupClient.getGroupByQuery(String.format("group==\"%s\"", action.getUpdated())).getUsergroups())) {
      ...
      case EXPIRATION_DATE:
        return user -> {
          ...
          try {
            ...
            throw new BulkOperationException(String.format("Invalid date format: %s, expected yyyy-MM-dd'T'HH:mm:ss.SSSXXX", action.getUpdated()));
          }
